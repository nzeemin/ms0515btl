...
int DebugView_DrawWatches(HDC hdc, int x, int y);
...
void DebugView_DoDraw(HDC hdc)
{
    ...
    int nWatches = DebugView_DrawWatches(hdc, xPorts + cxChar, cyLine / 2);
...
void DebugView_DrawMemoryMap(HDC hdc, int x, int y, const CProcessor* pProc)
{
    ...
    bool vramturedon = (port177400 & 0200) != 0;
    int vramwindow0 = -1, vramwindow1 = -1;
    if (vramturedon)
    {
        int vramflags = (port177400 >> 10) & 3;
        switch (vramflags)
        {
        case 0:
    ...
    for (uint16_t window = 0; window < 8; window++)
    {
        ...
        LPCTSTR szMapping = NULL;
        TCHAR buffer[10];
        if (window == 7)
            szMapping = _T("ROM");
        else if (window == 6 && (port177600 & 0200) != 0)
            szMapping = _T("ROM");
        else if (window == vramwindow0)
            szMapping = _T("VRAM0");
        else if (window == vramwindow1)
            szMapping = _T("VRAM1");
        if (szMapping == NULL)
        {
