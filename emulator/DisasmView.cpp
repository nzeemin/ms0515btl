...
BOOL DisasmView_GetJumpConditionHint(const uint16_t* memory, const CProcessor * pProc, LPTSTR buffer)
{
    ...
    *buffer = 0;
    ...
    if (instr >= 000200 && instr <= 000207)  // RTS / RETURN
    {
        ...
        int addrtype;
        uint16_t value = g_pBoard->GetWordView(spvalue, FALSE, &addrtype);
        ...
        (instr >= 0104000 && instr <= 0104777))  // TRAP, EMT
    {
        ...
        int addrtype;
        uint16_t value = g_pBoard->GetWordView(intvec, FALSE, &addrtype);
        ...
        int regnum, int regmod, bool byteword, uint16_t indexval)
{
    ...
    case 1:
    case 2:
        srcval2 = g_pBoard->GetWordView(regval, false, &addrtype);
        if (byteword)
        {
    ...
    case 4:
        ...
        {
            srcval2 = (regval & 1) ?
                    ((g_pBoard->GetWordView(regval - 1, false, &addrtype)) & 0xff) :
                    ((g_pBoard->GetWordView(regval - 2, false, &addrtype)) >> 8);
            _sntprintf(hint2, hintsize - 1, _T("(%s-1)=%03o"), REGISTER_NAME[regnum], srcval2);  // "(RN-1)=XXX"
        }
        else
        {
            srcval2 = g_pBoard->GetWordView(regval - 2, false, &addrtype);
    ...
    case 6:
        {
            ...
            srcval2 = g_pBoard->GetWordView(addr2 & ~1, false, &addrtype);
            if (byteword)
            {
    ...
    case 7:
        srcval2 = g_pBoard->GetWordView(regval + indexval, false, &addrtype);
        _sntprintf(hint2, hintsize - 1, _T("(%s+%06o)=%06o"), REGISTER_NAME[regnum], indexval, srcval2);  // "(RN+NNNNNN)=XXXXXX"
        //TODO: Show the real value in hint line 3
        break;
    }
        ...
        int regmod, bool byteword, uint16_t curaddr, uint16_t value)
{
    ...
    if (regmod == 3)
    {
        srcval2 = g_pBoard->GetWordView(value, false, &addrtype);
        if (byteword)
        {
    ...
    else if (regmod == 6)
    {
        ...
        srcval2 = g_pBoard->GetWordView(addr2, false, &addrtype);
        if (byteword)
        {
            srcval2 = (addr2 & 1) ? (srcval2 >> 8) : (srcval2 & 0xff);
        ...
        LPTSTR buffer, LPTSTR buffer2)
{
    ...
    *buffer = 0;  *buffer2 = 0;
    ...
    if ((instr & ~(uint16_t)077) == PI_MARK)
    {
        ...
        int addrtype = 0;
        uint16_t srcval2 = g_pBoard->GetWordView(regval, false, &addrtype);
...
void DisasmView_OnUpdate()
{
    ...
    for (int idx = 0; idx < nWindowSize; idx++)
    {
        memory[idx] = g_pBoard->GetWordView(
                (uint16_t)(current + idx * 2 - 10), FALSE, addrtype + idx);
    }
